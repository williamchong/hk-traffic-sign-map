...
// representative point; a street clause tests its name. `only` clips the
// districts to a ring and `minus` subtracts one — the rings may run outside
// the district freely, since only the intersection is ever used.
function clauseMatches(c, lng, lat, names) {
  if (c.streets) return names.some(n => n && c.streets.has(n))
...
// ray cast: inside any `polygons` entry, inside `only` if present, outside
// `minus` if present — or a `streets` name match.
//
...
export function shareableAreas(areas) {
  return {
    ...
    note: 'Generated by scripts/build-road-rules.mjs from data/taxi-zones/areas.json. A point is served by `taxi` at `access` when it is inside any `polygons` entry, inside `only` (if given) and outside `minus` (if given); a `streets` clause matches CENTERLINE.STREET_ENAME / ALIAS_ENAME instead. Most permissive access per colour wins: area > dest > route > none. Ring vertices are thinned to ~33 m.',
