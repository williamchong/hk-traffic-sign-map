...
// Taxis") on a handful of rows. The authority is Cap. 374E Sch. 7, which lists
// the permitted roads district by district: "all roads" in some, named roads
// and "between X and Y" stretches in others, with named exclusions. So the
// extent is curated in data/taxi-zones/areas.json, one clause per item of the
// Schedule, against the Home Affairs Department's district partition and the
// network's own street names, and classified onto CENTERLINE here.
//
...
export async function loadTaxiAreas(areasPath, districtsPath) {
  ...
  const clauses = areas.clauses.map((c, i) => {
    ...
    const except = (c.except ?? []).map((e) => {
      if (!e.streets && !e.only) throw new Error(`${where}: an \`except\` entry needs streets or only`)
      if (e.streets?.includes(UNNAMED) && !e.only) throw new Error(`${where}: an \`except\` "${UNNAMED}" needs an \`only\` ring`)
      return { streets: e.streets ? new Set(e.streets) : null, only: ringOf(e.only) }
    })
    return {
      ...c,
      geometries,
...
const namedIn = (streets, names) => names.some(n => n && streets.has(n)) || (streets.has(UNNAMED) && !names[0])

// Does this road satisfy one clause? A district clause tests the road's
// representative point; a street clause tests its name; a clause with both
// needs both, which is how Sch. 7 reads ("within Tsuen Wan District, the
// following roads"). `only` clips to a ring and `minus` subtracts one — the
// rings may run outside the district freely, since only the intersection is
// ever used; on a street a ring takes the stretch between two junctions. An
// `except` entry is one of the Schedule's own exclusions ("but not including
// Fei Ngo Shan Road"): a street list, a ring, or a street list inside a ring.
function clauseMatches(c, lng, lat, names) {
  if (c.streets && !namedIn(c.streets, names)) return false
  if (c.geometries.length) {
  ...
  for (const e of c.except) {
    if ((!e.streets || namedIn(e.streets, names)) && (!e.only || pointInRing(lng, lat, e.only))) return false
  }
  return true
}
...
export function shareableAreas(areas) {
  return {
    ...
    note: 'Generated by scripts/build-road-rules.mjs from data/taxi-zones/areas.json. A road is served by `taxi` at `access` when it is inside any `polygons` entry (if given) AND on a road whose CENTERLINE.STREET_ENAME / ALIAS_ENAME is listed in `streets` (if given; "(unnamed)" = a road with no STREET_ENAME), inside `only` (if given), outside `minus` (if given), and matched by no `except` entry (each one a `streets` list, an `only` ring, or both). Most permissive access per colour wins: area > dest > route > none. Ring vertices are thinned to ~33 m.',
    clauses: areas.clauses.map(c => ({
      ...
      ...(c.except.length ? { except: c.except.map(e => ({ ...(e.streets ? { streets: [...e.streets] } : {}), ...(e.only ? { only: e.only } : {}) })) } : {})
    }))
  }
}
