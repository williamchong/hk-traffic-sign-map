...
3. `build-tiles.mjs` → `ogr2ogr` reprojects each layer **EPSG:2326 (HK1980 Grid) → EPSG:4326**, injects a `category` property per the `sign-layers.mjs` manifest *and* (for the `traffic-sign-abbreviation` layer only) the `FACE_BEARING` looked up by `FEATUREID` from step 2, then `tippecanoe` packs everything into one `public/data/traffic-signs.pmtiles`. Source id and source-layer are both `signs`. tippecanoe maxzoom settles at 15 (MapLibre overzooms past it — fine for points).
