...
- **Sign icons are not rotated** even though TD features carry an `ANGLE`. Reading the dataspec it's the MicroStation symbol-cell rotation ("same as Ustn angle"), and the data confirms the consequence: 59% of same-code sign pairs within 30m share the same ANGLE, only 0.5% differ by ~180°, and the field's distribution is clamped to keep symbols upper-half readable. So both carriageways of a divided road carry one road-aligned ANGLE — feeding it to `icon-rotate` produces visually identical icons on opposite-bound lanes, which is misleading. An earlier commit (`fde0258`) wired this up and was reverted once the issue surfaced on real maps. Recovering face direction would need build-time inference from `DTAD_RD_AL_LINE` road centrelines and is a separate project.
